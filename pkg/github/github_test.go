...
import (
	...
	gh "github.com/sizocompany/sizo-db/pkg/github"
)
...
func TestClient_UploadReleaseAssets(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			files: map[string][]byte{
				"sizo.db.gz":       []byte("full"),
				"sizo-light.db.gz": []byte("light"),
			},
			filePaths: []string{
				"sizo.db.gz",
				"sizo-light.db.gz",
			...
			uploadReleaseAsset: []gh.UploadReleaseAssetExpectation{
				{
					Args: gh.UploadReleaseAssetArgs{
						Ctx:          context.Background(),
						Id:           1,
						Opt:          &github.UploadOptions{Name: "sizo.db.gz", Label: "", MediaType: "application/gzip"},
						FileAnything: true,
					},
					Returns: gh.UploadReleaseAssetReturns{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			dir, err := ioutil.TempDir("", "sizo-db")
